...
import 'package:fu_mobile/deedAppointment/cancelAppointment.dart';
...
class _detailState extends State<TransactionDetail> {
    ...
    Widget errorBox() => Wrap(
          children: [
            Center(
              ...
              children: [
                Text(
                  'Bu işleme ait bildirim bulunmamaktadır.',
  ...
  showAlertDialog(BuildContext context) {
    ...
    Widget okButton(islem, index, no, photo, fileName, refno, date) => SizedBox(
          child: Column(
            children: [
              Container(
                width: 200,
              ...
              Padding(
                ...
                child: TextButton(
                  ...
                  onPressed:
                      islem != "İşleme atanan bankaya tanım yapılmamıştır."
                          ? () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                ...
                if (snapshot.connectionState == ConnectionState.done) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: ((context, index) => appActions.length != 0
                        ? okButton(
                            appActions[index].Adi,
                            index,
                            appActions[index].No,
                            appActions[index].IsPhoto,
                            appActions[index].FileName,
                            widget.furefno,
                            date.text.length < 2
                                ? ""
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return FutureBuilder<String>(
            future: InsertLawyerAppointmentService(
                refno, time, year, month, day, widget.category),
            builder: ((context, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                if (snapshot.hasData) {
                  if (snapshot.data == "OK") {
                    ...
                    onTap: (textFirst == "Tapu Randevu Tarihi"
                        ? () async {
                            ...
                            if (pickedDate != null) {
                              ...
                              setState(() {
                                date.text =
                                    "${formatted2date}T${selectedTime.hour.toString().length == 2 ? selectedTime.hour : "0${selectedTime.hour}"}:${selectedTime.minute}:00+3:00";
                ...
                Padding(
                  ...
                  child: Padding(
                    ...
                    child: Container(
                      ...
                      child: Center(
                          child: Text(
                        "${(widget.category == "99" ? "Diğer" : widget.category == "1" ? "Tapu Randevusu alınmış" : widget.category == "2" ? "İpotek Evrağı gönderilmiş" : widget.category == "3" ? "Evrağı Gönderilmiş diğer" : widget.category == "4" ? "Gönderilmeyen" : "Gönderilmeyen")}",
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 16,
